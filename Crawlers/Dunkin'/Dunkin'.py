# -*- coding: utf-8 -*-
"""
Created on Sat Mar  6 15:47:41 2021
...
def QueryData():
    ...
    conn.request("GET", "/food/menuItems/search?query=Dunkin'&offset=0&number=800&minCalories=0&maxCalories=5000&minProtein=0&maxProtein=100&minFat=0&maxFat=100&minCarbs=0&maxCarbs=100", headers=headers)
    ...
    for item in data["menuItems"]:
        #if menu item comes from restaurant Chain Dunkin add id to id_list.
        if item["servingSize"] == None:
           dunkin_id_list.append(item["id"])
    #go through the list and make a menu item info request
    for id in dunkin_id_list:
        ...
        item_info['restaurant'] = "Dunkin'Donuts"
        #change manually 
...
def exportQuery(menu_items) -> str:
        ...
        with open("Dunkin'"+'-Query.txt', 'w') as fd:
            fd.write(query)
        return query
